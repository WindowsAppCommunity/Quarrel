...
namespace Discord.API.Sockets
{
    internal class WebSocketClient : IDisposable
    {
        ...
        {
            Guard.IsNotNull(_client, nameof(_client));
            ...
            try
            {
                while (!cancelToken.IsCancellationRequested)
                {
                    ...
                    {
                        throw new WebSocketClosedException((int)socketResult.CloseStatus.GetValueOrDefault(), socketResult.CloseStatusDescription);
                    }
                    ...
                    if (socketResult.MessageType == WebSocketMessageType.Text)
                    {
                        ...
                        TextMessage?.Invoke(text);
                    }
                    else
                    {
                        BinaryMessage?.Invoke(result, 0, resultCount);
                    }
                }
            }
