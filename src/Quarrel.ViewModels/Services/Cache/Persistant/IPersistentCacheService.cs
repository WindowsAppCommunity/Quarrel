...
namespace Quarrel.ViewModels.Services.Cache.Persistent
{
    ...
    public interface IPersistentCacheService
    {
        ...
        /// <returns>The contents of a remote file.</returns>
        [ItemNotNull]
        Task<object> GetRemoteFileAsync([NotNull] string url);
    }
    ...
    public interface ICacheStorageProvider
    {
        ...
        /// <returns>The stored value for a given key.</returns>
        [ItemCanBeNull]
        Task<T> TryGetValueAsync<T>([NotNull] string key, [CanBeNull] string scope = null)
            where T : class, new();
        ...
        /// <returns>The stored values for a collection of keys.</returns>
        [ItemNotNull]
        Task<IReadOnlyList<T>> TryGetValuesAsync<T>([NotNull] string scope)
            where T : class, new();
        ...
        Task SetValueAsync<T>([NotNull] string key, [NotNull] T value, [CanBeNull] string scope = null)
            where T : class, new();
