...
namespace Discord_UWP.CacheModels
{
    public class GuildChannel : INotifyPropertyChanged
    {
        public GuildChannel(SharedModels.GuildChannel input)
        {
            ...
            //            }
            //            else if (role.Name == "@everyone" && Storage.Cache.Guilds[input.GuildId].Members[Storage.Cache.CurrentUser.Raw.Id].Raw.Roles.Count() != 0)
            //            {
            //                Storage.Cache.Guilds[input.GuildId].perms.GetPermissions(role, Storage.Cache.Guilds[input.GuildId].RawGuild.Roles);
            //            }
            //            else
            //            {
