...
namespace RuntimeComponent.Opus
{
    ...
    public class OpusDecoder : IDisposable
    {
        ...
        public static OpusDecoder Create(int outputSampleRate, int outputChannels)
        {
            ...
            IntPtr error;
            IntPtr decoder = Codec.opus_decoder_create(outputSampleRate, outputChannels, out error);
        ...
        public unsafe float[] Decode([ReadOnlyArray()]byte[] inputOpusData, int dataLength, out int decodedLength)
        {
            if (disposed)
            ...
            fixed (float* bdec = decoded)
            {
                ...
                if (inputOpusData != null)
                    length = Codec.opus_decode(_decoder, inputOpusData, dataLength, decodedPtr, frameCount, 0);
                else
                    length = Codec.opus_decode(_decoder, null, 0, decodedPtr, frameCount, (ForwardErrorCorrection) ? 1 : 0);
            }
        ...
        public void Dispose()
        {
            ...
            {
                Codec.opus_decoder_destroy(_decoder);
