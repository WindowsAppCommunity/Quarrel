...
namespace RuntimeComponent.Opus
{
    /// <summary>
    /// Opus codec wrapper.
    /// </summary>
    public class OpusEncoder : IDisposable
    {
        ...
        public static OpusEncoder Create(int inputSamplingRate, int inputChannels, Application application)
        {
            ...
            IntPtr error;
            IntPtr encoder = Codec.opus_encoder_create(inputSamplingRate, inputChannels, (int)application, out error);
        ...
        public unsafe byte[] Encode([ReadOnlyArray()]float[] inputPcmSamples, int sampleLength, out int encodedLength)
        {
            if (disposed)
            ...
            fixed (byte* benc = encoded)
            {
                ...
                length = Codec.opus_encode(_encoder, inputPcmSamples, frames, encodedPtr, sampleLength);
            }
        ...
        /// <returns></returns>
        public int FrameCount([ReadOnlyArray()]float[] pcmSamples)
        {
        ...
        public int Bitrate
        {
            get
            {
                if (disposed)
                    throw new ObjectDisposedException("OpusEncoder");
                int bitrate;
                var ret = Codec.opus_encoder_ctl(_encoder, Ctl.GetBitrateRequest, out bitrate);
                if (ret < 0)
            ...
            set
            {
                if (disposed)
                    throw new ObjectDisposedException("OpusEncoder");
                var ret = Codec.opus_encoder_ctl(_encoder, Ctl.SetBitrateRequest, value);
                if (ret < 0)
                    throw new Exception("Encoder error - " + ((Errors)ret).ToString());
            }
        }
        ...
        public bool ForwardErrorCorrection
        {
            get
            {
                ...
                int fec;
                int ret = Codec.opus_encoder_ctl(_encoder, Ctl.GetInbandFECRequest, out fec);
                if (ret < 0)
                    throw new Exception("Encoder error - " + ((Errors) ret).ToString());
            ...
            set
            {
                ...
                var ret = Codec.opus_encoder_ctl(_encoder, Ctl.SetInbandFECRequest, value ? 1 : 0);
                if (ret < 0)
                    throw new Exception("Encoder error - " + ((Errors) ret).ToString());
            }
        }
        ...
        public void Dispose()
        {
            ...
            if (_encoder != IntPtr.Zero)
            {
                Codec.opus_encoder_destroy(_encoder);
