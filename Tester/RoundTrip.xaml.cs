...
namespace Tester
{
    ...
    public sealed partial class RoundTrip : Page
    {
        ...
        {
            encoder = OpusEncoder.Create(48000, 2, RuntimeComponent.Application.Voip);
        ...
        private void RTProcess(object sender, float[] e)
        {
            try
            {
                ...
                int encodedSize;
                byte[] opus = encoder.Encode(e, 960, out encodedSize);
                ...
                int samples;
                float[] frame = decoder.Decode(opus, encodedSize, out samples);
