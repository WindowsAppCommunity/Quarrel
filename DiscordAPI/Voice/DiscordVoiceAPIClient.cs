...
namespace Discord_UWP.Voice
{
    ...
    internal class DiscordVoiceAPIClient
    {
        ...
        public event Func<OperationCode, Task> SentGatewayMessage { add { _sentGatewayMessageEvent.Add(value); } remove { _sentGatewayMessageEvent.Remove(value); } }
        private readonly AsyncEvent<Func<OperationCode, Task>> _sentGatewayMessageEvent = new AsyncEvent<Func<OperationCode, Task>>();
        ...
        public event Func<OperationCode, object, Task> ReceivedEvent { add { _receivedEvent.Add(value); } remove { _receivedEvent.Remove(value); } }
        private readonly AsyncEvent<Func<OperationCode, object, Task>> _receivedEvent = new AsyncEvent<Func<OperationCode, object, Task>>();
            ...
            WebSocketClient.BinaryMessage += async (data, index, count) =>
            {
                ...
                using (var decompressed = new MemoryStream())
                {
                    ...
                    using (var reader = new StreamReader(decompressed))
                    {
                        ...
                        await _receivedEvent.InvokeAsync((OperationCode)msg.Operation, msg.Payload).ConfigureAwait(false);
                    }
                }
            };
        ...
        public async Task SendAsync(OperationCode opCode, object payload, RequestOptions options = null)
        {
            byte[] bytes = null;
        ...
        {
            await SendAsync(OperationCode.Heartbeat, DateTimeUtils.ToUnixMilliseconds(DateTimeOffset.UtcNow), options: options).ConfigureAwait(false);
        }
        public async Task SendIdentityAsync(ulong userId, string sessionId, string token)
        {
            await SendAsync(OperationCode.Identify, new Identify
            {
                GuildId = GuildId,
                UserId = userId,
                SessionId = sessionId,
        ...
        {
            await SendAsync(OperationCode.SelectProtocol, new SelectProtocolParams
            {
                Protocol = "udp",
        ...
        {
            await SendAsync(OperationCode.Speaking, new Speaking
            {
                IsSpeaking = value,
                Delay = 0
            }).ConfigureAwait(false);
